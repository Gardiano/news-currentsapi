...
export const Search = () => {
  const { value, handleChange, loading } = useSearch();

  return (
    <div className="w-full max-w-[1144px] h-full hidden flex-row justify-center mx-auto lg:flex">
      <div className="w-[calc(100%-40px)] h-fit flex flex-row justify-center items-center">
        ...
        <Button
          ...
          className="w-[64px] h-[36px] m-0 p-0 bg-zinc-600 
          rounded-tl-none rounded-bl-none hover:bg-zinc-600 sm:h-[42px]">
          <NavigationLink
            url={`/news/searched/${value}`}
            className="w-full h-full flex items-center justify-center text-white 
              text-xs bg-transparent
             sm:bg-zinc-600 rounded-none sm:rounded-tr-sm sm:rounded-br-sm">
              {!loading ? <SearchIcon className="w-[44px] h-[16px] sm:bg-zinc-600" /> : <Loader className="animate-spin" />}
          </NavigationLink>
        </Button>
      </div>
    </div>
